...
pub const interrupts = struct {
    pub fn SysTick() void {
        clock.handleTickInterrupt();
        const mod = clock.current_tick.raw & 0x3F;
        if (mod == 0) {
            SlowClock.modifyInline(0);
        } else if (mod == 31) {
            SlowClock.modifyInline(1);
        }
    }
};
...
fn readChip() !void {
    ...
    const idcode = doCommand(.IDCODE, u32, 0xFFFF_FFFF);
    if (idcode != 0x1180C043) {
        return error.BadIDCode;
    }

    _ = doCommand(.SAMPLE_PRELOAD, u138, 0);
