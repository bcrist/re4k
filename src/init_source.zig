...
fn runToolchain(ta: std.mem.Allocator, tc: *Toolchain, dev: *const DeviceInfo, mcref: common.MacrocellRef, pt3_ar: bool) !toolchain.FitResults {
    ...
    try helper.logResults(dev.device, "pt3_reset_glb{}_mc{}_{}", .{ mcref.glb, mcref.mc, pt3_ar }, results);
...
pub fn run(ta: std.mem.Allocator, pa: std.mem.Allocator, tc: *Toolchain, dev: *const DeviceInfo, writer: *sx.Writer(std.fs.File.Writer)) !void {
    ...
    try writer.expressionExpanded("init_source");
    ...
    while (mc_iter.next()) |mcref| {
        ...
        if (diff.countSet() != 1) {
            try helper.err("Expected one init_source fuse but found {}!", .{ diff.countSet() }, dev, .{ .mcref = mcref });
        }
